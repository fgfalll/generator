...
df = pd.read_excel(excel_path, dtype={"Реєстраційни номер":str,"Номер зно":str,"Рік зно":str}, sheet_name = "Лист1").fillna(value=' ')

#User imput
what = input('Що друкувати?\n')
...
if des == "так":
    ...
    df["baly_ukr_slova"] = df["ЗНО.Українська мова та література"].apply(num2words, lang='uk').mask(df[""])
